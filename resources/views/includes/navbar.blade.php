          ...
          <div class="relative flex items-center justify-between h-16">
              ...
              <div class="flex-1 flex items-center justify-center sm:items-stretch sm:justify-between">
                  <div class="flex-shrink-0 flex items-center">
                      <span class="text-xl font-bold text-white">Website Pengantin</span>
                  </div>
                  <div class="hidden sm:block sm:ml-6">
                      <div class="flex space-x-4">
                          @guest
                          @if (Route::has('login'))
                          <a href="{{ route('login') }}" class="text-blue-300 hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium">Masuk</a>
                          @endif
                          @if (Route::has('register'))
                          <a href="{{ route('register') }}" class="text-blue-300 outline outline-1 hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium">Daftar</a>
                          @endif
                          @else
                          <a href="#" class="bg-gray-900 text-white px-3 py-2 rounded-md text-sm font-medium" aria-current="page">Beranda</a>
                          @endguest
                      </div>
                  </div>
              </div>
