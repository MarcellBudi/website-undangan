...
<body>
  ...
  <section class="h-screen md:w-1/2 mx-auto">
      <div class="text-center mt-56 leading-10">
        ...
        <h1 class="text-2xl pb-6">{{ $undangan->tanggal }}</h1>
        <h2 class="text-7xl">{{ $undangan->pengantin_pria }} 
        <h2 class="text-3xl py-5">&</h2> 
        <h2 class="text-7xl">{{ $undangan->pengantin_wanita }} </h2>
      </div>
    ...
    <div class="mt-20 flex justify-center space-x-16 items-center flex-col md:flex-row md:mr-9">
      {{-- {{ URL(storage/prewedding/38b3eff8baf56627478ec76a704e9b52.jpg) }} --}}
        <figure class="text-center md:pl-32">
          <img class="w-60 rounded-full mx-auto" src="{{ URL($gambar_pengantin_pria->path) }}" alt="">
          <h1 class="text-center pt-5 text-xl">{{ $undangan->pengantin_pria }}</h1>
          <h2 class="py-5">Putra dari Pasangan<br>{{ $undangan->ortu_pengantin_pria }}</h2>
          <h2>Br. Penestanan Kaja, Ds. Sayan, Kec. Ubud, Kab. Gianyar</h2>
          </a>
        </figure>
        <figure class="text-center pt-24 md:pb-24 pr-16 md:pr-14 px-auto">
          <img class="w-60 rounded-full mx-auto" src="{{ URL($gambar_pengantin_wanita->path) }}" alt="">
          <h1 class="text-center pt-5 text-xl">{{ $undangan->pengantin_wanita }}</h1>
          <h2 class="py-5">Putra dari Pasangan<br>{{ $undangan->ortu_pengantin_wanita }}</h2>
          <h2>Br. Penestanan Kaja, Ds. Sayan, Kec. Ubud, Kab. Gianyar</h2>
        </figure>
    </div>
  </section>
  <section class="text-center mt-40 border-2 w-80 mx-auto border-solid border-green my-20">
      <div>
        ...
        <h3 class="text-lg">Monday, {{ $undangan->tanggal }}</h3>
        <h3 class="text-md">12:00 - 23:59 WITA</h3>
        <h4 class="my-5 text-bold">{{ $undangan->alamat }}</h4>
      </div>
  </section>
    ...
    <div class="">
      ...
      <figure class="flex gap-5 flex-wrap justify-center items-center my-20">
        @foreach ($gambar_galeri as $gg)
          <img class="w-60 rounded-sm" src="{{ URL($gg->path) }}" alt="">   
        @endforeach
      </figure>
    </div>
  </section>
  ...
  <section class="text-center mt-48 mb-9">
    <div class="">
